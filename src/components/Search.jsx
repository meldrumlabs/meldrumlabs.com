...
import {navItems as aboutNavItems} from '@/app/about/layout';
...
function HighlightQuery({text, query}) {
  return (
    <Highlighter
      highlightClassName="underline bg-transparent text-iroh-purple-500"
      searchWords={[query]}
      autoEscape={true}
...
function SearchResult({
  ...
  const id = useId();
  const navigation = [...aboutNavItems];
  ...
  return (
    ...
    >
      <div
        id={`${id}-title`}
        aria-hidden="true"
        className="text-sm font-medium text-zinc-900 group-aria-selected:text-iroh-purple-500 dark:text-white"
      >
    ...
    <div className="group relative flex h-12">
      ...
      {autocompleteState.status === 'stalled' && (
        <div className="absolute inset-y-0 right-3 flex items-center">
          <LoadingIcon className="h-5 w-5 animate-spin stroke-zinc-200 text-zinc-900 dark:stroke-zinc-800 dark:text-iroh-purple-400" />
        </div>
      )}
    </div>
  );
...
export function Search() {
  ...
  return (
    <div className="hidden lg:block lg:max-w-md lg:flex-auto">
      <button
        type="button"
        className="hidden h-8 w-full items-center gap-2 rounded-sm bg-white pl-2 pr-3 text-sm text-zinc-500 ring-1 ring-zinc-900/10 transition hover:ring-zinc-900/20 ui-not-focus-visible:outline-none dark:bg-white/5 dark:text-zinc-400 dark:ring-inset dark:ring-white/10 dark:hover:ring-white/20 lg:flex"
        {...buttonProps}
      >
